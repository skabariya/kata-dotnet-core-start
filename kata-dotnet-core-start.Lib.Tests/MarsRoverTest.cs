...
public class MarsRoverTest
{
    ...
    [Fact]
    public void WhenGivingCommand_Forward_Then_PositionShouldBe_0_3()
    {
        //Arrange
        var marsRover = new MarsRover();

        //Act
        var result = marsRover.Execute(new MoveForwardCommand());

        //Assert
        Assert.True(result);
        var currentPosition = marsRover.CurrentPosition();
        Assert.Equal((uint) 0, currentPosition.X);
        Assert.Equal((uint) 3, currentPosition.Y);
    }
}